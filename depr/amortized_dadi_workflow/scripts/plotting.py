...
with open(f"{posteriordir}/n_train_{n_train}/ensemble_posterior.pkl", "rb") as f:
...
np.save(f"{posteriordir}/n_train_{n_train}/default_obs_samples.npy", samples)
...
np.save(os.path.join(posteriordir, f"n_train_{n_train}/model_fs.npy"), model_fs)
np.save(os.path.join(posteriordir, f"n_train_{n_train}/map_thetas.npy"), map_thetas)
