...
# Set up config
configfile: "config/amortized_msprime/YRI_CEU_dinf_new.yaml"

n_ensemble = int(config["n_ensemble"]) # number of times to repeat SNPE training
n_train = int(config["n_train"]) # training set size
n_test = int(config["n_test"]) # test set size

project_dir = config["project_dir"]
train_sim_dir = os.path.join(project_dir, "sims", "train")
test_sim_dir = os.path.join(project_dir, "sims", "test")
posterior_dir = os.path.join(project_dir, "npe")
...
rule simulate_all:
    input:
        train = expand(
            os.path.join(train_sim_dir, "batch_{batch}", "done"), 
            batch=range((n_train + CHUNK_SIZE - 1) // CHUNK_SIZE),
        ),
        test = expand(
            os.path.join(test_sim_dir, "batch_{batch}", "done"), 
            batch=range((n_test + CHUNK_SIZE - 1) // CHUNK_SIZE),
        ),
        # Default simulation
