...
# Set up config
configfile: "config/PonAbe_IM_msprime.yaml"
...
rule all:
    input:
        ...
        expand(os.path.join(datadir, "sim_round_{k}/", "x_{i}.npy"), k=list(range(n_rounds)), i=range(n_sims_per_round)),
...
rule process_ts:
    ...
    input:
        os.path.join(datadir, "sim_round_{k}/", "{i}.trees")
    output:
        os.path.join(datadir, "sim_round_{k}/", "x_{i}.npy")
    log:
...
rule train_npe:
    ...
    input:
        lambda wildcards: expand(os.path.join(datadir, "sim_round_{k}/", "x_{i}.npy"), i=range(n_sims_per_round), k=[wildcards.k]),
