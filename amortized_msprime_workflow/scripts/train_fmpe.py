...
flowmatching_density_estimator = flowmatching_nn(
    model="mlp",
    z_score_theta="none",
    z_score_x="none",
...
inference = FMPE(
    prior=prior,
    density_estimator=flowmatching_density_estimator,
    device=device.type,
...
for n in range(int(sim_rounds)+1):
    ...
    inference = inference.append_simulations(thetas, xs, proposal=prior, data_device=device.type)
