...
# Set up config
configfile: "config/dadi/PonAbe_IM_sample.yaml"

n_sims_per_round = config["n_sims_per_round"] # number of simulations per round
n_rounds = 30 # number of rounds set by config. If the maximum number of rounds run so far is larger, replace it with that number.
