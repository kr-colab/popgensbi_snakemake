...
class ExchangeableCNN(nn.Module):
    ...
    #        y = genotypes[:, i]
    #        y = y[y != -1].reshape(y.shape[0], y.shape[1], -1, y.shape[-1])
    #        print(y.shape)
    #        assert False
    #        x.append(self.symmetric(self.cnn(y)))
    #    x = torch.cat(x, dim=-1)
    #    x = self.globalpool(x)
...
class RNN(nn.Module):
    def __init__(self, input_size, output_size, num_layers=2, dropout=0.35):
        """
        ...
        self.fc1 = nn.Sequential(nn.Linear(168 * num_layers, 256), nn.Dropout(dropout))
    ...
    def forward(self, x):
        ...
        hapout = self.fc1(hn)
        return self.feature_ext(hapout)

    def embedding(self, x):
        """ last hidden layer ignoring dropout """
        with torch.no_grad():
            _, hn = self.rnn(x)
            hn = hn.permute(1, 0, 2).reshape(hn.shape[1], -1)
            hapout = self.fc1(hn)
            return self.feature_ext[0](hapout)
